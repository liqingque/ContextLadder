...
在一个已被证明「未见化合物信息不可得」的任务上，把「如何优雅地不知道」当作一等设计目标——让模型的每一级上下文依赖都可以逐级回退，宁可退到更粗的一级，也不在没有证据的地方假装知道。
...
「阶梯」指本方案的核心设计原则：把模型对实验条件的依赖组织成一道**可以逐级回退的阶梯**。测量上下文中，板号是仪器与来源之上的收缩残差，遇到未见板时自动退回上一级；化学扰动上，训练时随机掩码 25% 的化合物 token，迫使模型在未见化合物上退回生物与测量上下文的先验。两个创新点是同一原则的两次实例化——**宁可退到更粗的一级，也不要在没有证据的地方假装知道**。这正对应赛题的四个泛化场景（未见化合物、未见菌株、双重未知、时间插值）。
